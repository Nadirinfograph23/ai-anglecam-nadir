...
import {
  generateAllAnglesStream,
  retryAngle as retryAngleService,
  PREDEFINED_ANGLES,
  type AngleResult,
} from "./services/gradio-client";
...
const ANGLE_NAMES = PREDEFINED_ANGLES.map((a) => a.name);
...
function App() {
  ...
  const generateAllAngles = async () => {
    ...
    try {
      await generateAllAnglesStream(imageFile, lens, {
        onStart: (total) => {
  ...
  const retryAngle = async (angleName: string) => {
    ...
    try {
      const result = await retryAngleService(imageFile, angleName, lens);
      setResults((prev) => {
        const existing = prev.filter((r) => r.name !== angleName);
        return [...existing, result];
      });
      if (!result.success) {
        setError("Retry for " + angleName + " failed: " + (result.error || "Unknown error"));
      }
    } catch (e) {
